...
(async()=>{
 ...
 try {
  ...
  await page.click('[data-learning-help="example"]');
  assert.match(await page.locator('#learningHelpContent').innerText(),/Milk/);
  await page.click('.learning-help-sheet button[aria-label="关闭帮助"]');
  ...
  await page.click('#helpButton');
  await page.click('[data-learning-help="example"]');
  assert.match(await page.locator('#learningHelpContent').innerText(),/A little more, please/);
  await page.click('.learning-help-sheet button[aria-label="关闭帮助"]');
  assert.equal(await page.locator('#breakfastChoices button').count(),0);
